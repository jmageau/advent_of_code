...
fn answer_two() -> String {
    ...
    for i in 0..(lines.len()) / 3 {
        let numbers_1: Vec<_> = lines[i * 3]
            .split(' ')
            .map(|s| s.parse::<u16>().unwrap())
            .collect();
        let numbers_2: Vec<_> = lines[i * 3 + 1]
            .split(' ')
            .map(|s| s.parse::<u16>().unwrap())
            .collect();
        let numbers_3: Vec<_> = lines[i * 3 + 2]
            .split(' ')
            .map(|s| s.parse::<u16>().unwrap())
            .collect();
...
fn is_valid_triangle(dimensions: &[u16]) -> bool {
    let sum: u16 = dimensions.iter().sum();
    let max: u16 = *dimensions.iter().max().unwrap();
    sum > max * 2
}
